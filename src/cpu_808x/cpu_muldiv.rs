...
pub trait Cord<B = Self>: Sized {
    fn cord(self, cpu: &mut Cpu, b: B, c: B) -> Result<(u16, u16, bool), bool>;
}
...
macro_rules! impl_cord {
    ($prim:ty) => {
        impl Cord for $prim {
            ...
            fn cord(self, cpu: &mut Cpu, b: Self, c: Self) -> Result<(u16, u16, bool), bool> {

                let mut internal_counter;
            
                let mut tmpa: u16 = self as u16;
                ...
                while internal_counter > 0 { 
                    ...
                    // 18d:
                    tmpa = sigma_s as u16;
                    (sigma, carry_sub, _, _) = tmpa.alu_sub(tmpb);
                ...
                (sigma_s, _) = (tmpc as Self).alu_rcl(1, carry);
                ...
                cpu.cycles_i(3, &[0x193, 0x194, MC_JUMP]);
...
macro_rules! impl_cor_negate {
    ($prim:ty) => {
        impl CorNegate for $prim {
            ...
            fn cor_negate(self, cpu: &mut Cpu, mut tmpb: u16, mut tmpc: u16, mut neg_flag: bool, skip: bool) -> (u16, u16, u16, bool, bool) {

                let mut tmpa = self as i8 as i16 as u16; // Sign-extend
                let mut sigma: u16;
                let mut sigma_s: u8;
                let mut carry: bool;
                let next_carry: bool;
...
impl<'a> Cpu<'a> {
    ...
    pub fn corx8(&mut self, b: u8, c: u8, mut carry: bool) -> (u16, u16) {
        ...
        loop { 
            ...
            println!("  >> corx8: tmpa: {} tmpc: {}", tmpa, tmpc);
        ...
        // Fall through line 186
        self.cycles_i(2, &[0x187, MC_RTN]); // 187 'RTN', return delay cycle
    ...
    pub fn div8(&mut self, dividend: u16, divisor: u8, signed: bool, mut negate: bool) -> Result<(u8, u8), bool> {
        ...
        println!("  div8: a: {:04x}, b: {:04x}, c: {:04x}, n: {}", tmpa, tmpb, tmpc, negate);
        ...
        if signed {
            ...
            println!("  div8: post-negate: a: {:04x}, b: {:04x}, c: {:04x}, n: {}", tmpa, tmpb, tmpc, negate);
        }
    
        // 163
        self.cycles_i(2, &[0x163, MC_JUMP]);
        (tmpc, tmpa, carry) = match (tmpa as u8).cord(self, tmpb as u8, tmpc as u8) {
        ...
        if signed {
            ...
            println!("  div8: POSTIDIV");

            // 1c4:         | NCY INT0
            if !carry {
                self.cycles_i(2, &[0x1c4, MC_JUMP]);
                return Err(false);
            }
            ...
            self.cycles_i(4, &[0x1cB4, 0x1c5, 0x1c6, 0x1c7]);
            ...
            if !carry {
                ...
                println!("  div8: tmpb was negative in POSTIDIV");
            ...
            if !negate {
                ...
                println!("  div8: negate flag not set: tmpc = !tmpc");
                sigma16 = !tmpc; // 1cb:        | COM tmpc
                self.cycle_i(0x1cb);
            }
            else {
                println!("  div8: negate flag was set: tmpc = NEG tmpa + 1");
                self.cycle_i(MC_JUMP);
            }
        ...
        println!("  div8: done: a: {} b: {} c:{} ", tmpa, tmpb, tmpc);
    ...
    pub fn div16(&mut self, dividend: u32, divisor: u16, signed: bool, mut negate: bool) -> Result<(u16, u16), bool> {
        ...
        println!("  div16: a: {:04x}, b: {:04x}, c: {:04x}, n: {}", tmpa, tmpb, tmpc, negate);
        ...
        if signed {
            ...
            println!("  div16: post-negate: a: {:04x}, b: {:04x}, c: {:04x}, n: {}", tmpa, tmpb, tmpc, negate);
        }
    
        // 16b:
        self.cycles_i(2, &[0x163, MC_JUMP]);
        (tmpc, tmpa, carry) = match tmpa.cord(self, tmpb, tmpc) {
            Ok((tmpc, tmpa, carry)) => (tmpc, tmpa, carry),
            Err(_) => return Err(false)
        };
        //sigma16 = tmpa;
        ...
        if signed {
            ...
            if !carry {
                ...
                // divisor is negative
                println!("  div16: tmpb was negative in POSTIDIV");
            ...
            if !negate {
                ...
                println!("  div16 negate flag not set: COM tmpc");
                sigma16 = !tmpc; // 1cb:        | COM1 tmpc
                self.cycle_i(0x1cb);
            }
            else {
                println!("  div16: negate flag was set");
                self.cycle_i(MC_JUMP);
            }
        ...
        println!("  div16: done: a: {} b: {} c:{} ", tmpa, tmpb, tmpc);
